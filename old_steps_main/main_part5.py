...
def home(request: Request, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.Post)) # query the database for all post in database
    posts = result.scalars().all() # take all posts and return in template
...
def post_page(request: Request, post_id: int, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.Post).where(models.Post.id == post_id)) # query the database
    post = result.scalars().first() # take the post
...
# Endpoint with database in order to create new user
